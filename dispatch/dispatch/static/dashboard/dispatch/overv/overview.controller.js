    ...
    $scope.allRouters = {
      ...
      onRegisterApi: function(gridApi){
        gridApi.selection.on.rowSelectionChanged($scope,function(row){
          setActivated('router' , row.entity.nodeId, 'nodeId')
        });
      }
    };
...
//QDR.log.debug("routerInfo done updating fields")
//console.dump($scope.routerFields)
    ...
    $scope.addressesGrid = {
      ...
      onRegisterApi: function(gridApi){
        gridApi.selection.on.rowSelectionChanged($scope,function(row){
          setActivated('address', row.entity.uid, 'uid')
        });
      }
    };
...
*/
    ...
    $scope.linksGrid = {
        ...
        },
        {
          ...
          menuItems: [showHideConsoleMenuItem,
          {
            ...
            action: function($event) {
              ...
              this.context.col.colDef.menuItems[1].title =
                endpointLinksTitle()
      ...
      onRegisterApi: function(gridApi){
        gridApi.selection.on.rowSelectionChanged($scope,function(row){
          setActivated('link', row.entity.uid, 'uid')
        });
...
//      loadColState($scope.linksGrid);
    ...
    $scope.allConnectionGrid = {
      ...
      onRegisterApi: function(gridApi){
        gridApi.selection.on.rowSelectionChanged($scope,function(row){
          setActivated('connection', row.entity.uid, 'uid')
        });
      }
    };
    ...
    $scope.allLogGrid = {
      ...
      onRegisterApi: function(gridApi){
        gridApi.selection.on.rowSelectionChanged($scope,function(row){
          setActivated('log', row.entity.module, 'module')
        });
      }
    };
    ...
    var setActivated = function (dropdown, uid, suid) {
...
//console.dump($scope.selectedObject)
    ...
    var updateLinkTree = function (linkFields) {
      updateDropdown('link', linkFields, 'uid', 'title', allLinkInfo, 'links', linkInfo)
    }
    ...
    initDropDown('link', allLinkInfo, 'links', true)
...
QDR.log.debug("initTreeState called after connected");
      allRouterInfo();
      allAddressInfo();
      allLinkInfo();
      allConnectionInfo()
  ...
  QDR.log.debug("dataReady");
    ...
    if (!QDRService.connected) {
      QDRService.addConnectAction(initTreeState);
      QDRService.connect();
    } else {
      QDR.log.debug("we were already connected?");
      initTreeState();
    }
